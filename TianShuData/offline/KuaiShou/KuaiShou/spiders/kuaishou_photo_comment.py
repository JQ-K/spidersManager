...
from KuaiShou.items import KuaishouPhotoCommentItem
...
class KuaishouPhotoCommentSpider(scrapy.Spider):
    ...
    def __init__(self, partitionIdx='0', useProxy='0', cookieManual='0', cookieIdx='0', *args, **kwargs):
        ...
        self.saveFilePath = self.settings.get('FILE_PATH_PHOTO_COMMENT')
    ...
    def start_requests(self):
        ...
        #                      callback=self.parseCommentList, meta={'photoId': photoId})
        ...
        for message in consumer:
            try:
                ...
                if msg_value_dict['spider_name'] != 'kuaishou_public_feeds':
                    continue
    ...
    def parseCommentList(self, response):
        ...
        if 'shortVideoCommentList' not in rlt_json['data']:
            logger.info('shortVideoCommentList not in response data, photoId: ' + str(photoId))
            return

        if rlt_json['data']['shortVideoCommentList'] is None:
            logger.info('shortVideoCommentList is None, photoId: ' + str(photoId))
            return

        shortVideoCommentList = rlt_json['data']['shortVideoCommentList']
        if 'commentList' not in shortVideoCommentList:
            logger.info('commentList not in shortVideoCommentList, photoId: ' + str(photoId))
            return
        if shortVideoCommentList['commentList'] is None:
            logger.info('commentList is None, photoId: ' + str(photoId))
            return

        commentList = shortVideoCommentList['commentList']
        for comment in commentList:
            commentInfo = KuaishouPhotoCommentItem()
            ...
            if len(comment['subComments']) > 0:
                commentInfo['hasSubComment'] = True
            else:
                commentInfo['hasSubComment'] = False
